 ...
 *
 * cronicle is freely distributable under the MIT license.
 * For all details and documentation:
 *     http://github.com/bunkat/cronicle
 */

cronicle.array.sort = function (arr, zeroIsLast) {
...
cronicle.array.next = function (val, values, extent) {
  var cur
...
cronicle.array.nextInvalid = function (val, values, extent) {
...
cronicle.array.prev = function (val, values, extent) {
...
cronicle.array.prevInvalid = function (val, values, extent) {
  var min = extent[0]; var max = extent[1]; var len = values.length
...
cronicle.day = cronicle.D = {
   ...
   */
  val: function (d) {
    return d.D || (d.D = cronicle.date.getDate.call(d))
  },
  ...
  isValid: function (d, val) {
    return cronicle.D.val(d) === (val || cronicle.D.extent(d)[1])
  },
  ...
  extent: function (d) {
    ...
    var month = cronicle.M.val(d)

    var max = cronicle.DAYS_IN_MONTH[month - 1]

    if (month === 2 && cronicle.dy.extent(d)[1] === 366) {
      max = max + 1
    }
   ...
   */
  start: function (d) {
    return d.DStart || (d.DStart = cronicle.date.next(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d)))
  },
   ...
   */
    ...
    return d.DEnd || (d.DEnd = cronicle.date.prev(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d)))
  },
   ...
   */
  next: function (d, val) {
    val = val > cronicle.D.extent(d)[1] ? 1 : val
    var month = cronicle.date.nextRollover(d, val, cronicle.D, cronicle.M)

    var DMax = cronicle.D.extent(month)[1]

    val = val > DMax ? 1 : val || DMax

    return cronicle.date.next(
      cronicle.Y.val(month),
      cronicle.M.val(month),
      val
    )
  },
   ...
   */
  prev: function (d, val) {
    var month = cronicle.date.prevRollover(d, val, cronicle.D, cronicle.M)
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.dayOfWeekCount = cronicle.dc = {
   ...
   */
  val: function (d) {
    return d.dc || (d.dc = Math.floor((cronicle.D.val(d) - 1) / 7) + 1)
  },
   ...
   */
  isValid: function (d, val) {
    return (cronicle.dc.val(d) === val) ||
      (val === 0 && cronicle.D.val(d) > cronicle.D.extent(d)[1] - 7)
  },
   ...
   */
  extent: function (d) {
    return d.dcExtent || (d.dcExtent = [1, Math.ceil(cronicle.D.extent(d)[1] / 7)])
  },
  ...
  start: function (d) {
    return d.dcStart || (d.dcStart =
      cronicle.date.next(
        cronicle.Y.val(d),
        cronicle.M.val(d),
        Math.max(1, ((cronicle.dc.val(d) - 1) * 7) + 1 || 1)))
  },
   ...
   */
    ...
    return d.dcEnd || (d.dcEnd =
      cronicle.date.prev(
        cronicle.Y.val(d),
        cronicle.M.val(d),
        Math.min(cronicle.dc.val(d) * 7, cronicle.D.extent(d)[1])))
  },
   ...
   */
  next: function (d, val) {
    val = val > cronicle.dc.extent(d)[1] ? 1 : val
    var month = cronicle.date.nextRollover(d, val, cronicle.dc, cronicle.M)

    var dcMax = cronicle.dc.extent(month)[1]

    val = val > dcMax ? 1 : val

    var next = cronicle.date.next(
      cronicle.Y.val(month),
      cronicle.M.val(month),
      val === 0 ? cronicle.D.extent(month)[1] - 6 : 1 + (7 * (val - 1))
    )

    if (next.getTime() <= d.getTime()) {
      month = cronicle.M.next(d, cronicle.M.val(d) + 1)
   ...
   */
  prev: function (d, val) {
    var month = cronicle.date.prevRollover(d, val, cronicle.dc, cronicle.M)

    var dcMax = cronicle.dc.extent(month)[1]

    val = val > dcMax ? dcMax : val || dcMax

    return cronicle.dc.end(cronicle.date.prev(
      cronicle.Y.val(month),
      cronicle.M.val(month),
      1 + (7 * (val - 1))
    ))
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.dayOfWeek = cronicle.dw = cronicle.d = {
   ...
   */
  val: function (d) {
    return d.dw || (d.dw = cronicle.date.getDay.call(d) + 1)
  },
   ...
   */
  isValid: function (d, val) {
    return cronicle.dw.val(d) === (val || 7)
  },

  /**
   * The minimum and maximum valid day of week values. Unlike the standard
   * Date object, cronicle day of week goes from 1 to 7.
   */
  extent: function () {
    return [1, 7]
  },
  ...
  next: function (d, val) {
    ...
    return cronicle.date.next(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      cronicle.D.val(d) + (val - cronicle.dw.val(d)) + (val <= cronicle.dw.val(d) ? 7 : 0))
  },
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.dayOfYear = cronicle.dy = {
  ...
  val: function (d) {
    return d.dy || (d.dy =
      Math.ceil(1 + (cronicle.D.start(d).getTime() - cronicle.Y.start(d).getTime()) / cronicle.DAY))
  },
   ...
   */
  isValid: function (d, val) {
    return cronicle.dy.val(d) === (val || cronicle.dy.extent(d)[1])
  },
   ...
   */
  next: function (d, val) {
    val = val > cronicle.dy.extent(d)[1] ? 1 : val
    var year = cronicle.date.nextRollover(d, val, cronicle.dy, cronicle.Y)

    var dyMax = cronicle.dy.extent(year)[1]
   ...
   */
  prev: function (d, val) {
    var year = cronicle.date.prevRollover(d, val, cronicle.dy, cronicle.Y)
...
cronicle.hour = cronicle.h = {
   ...
   */
  val: function (d) {
    return d.h || (d.h = cronicle.date.getHour.call(d))
  },
   ...
   */
  start: function (d) {
    return d.hStart || (d.hStart = cronicle.date.next(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d), cronicle.h.val(d)))
  },
   ...
   */
    ...
    return d.hEnd || (d.hEnd = cronicle.date.prev(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d), cronicle.h.val(d)))
  },
  ...
  next: function (d, val) {
    ...
    var next = cronicle.date.next(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      cronicle.D.val(d) + (val <= cronicle.h.val(d) ? 1 : 0),
      val)

    // correct for passing over a daylight savings boundry
    if (!cronicle.date.isUTC && next.getTime() <= d.getTime()) {
      next = cronicle.date.next(
        cronicle.Y.val(next),
        cronicle.M.val(next),
        cronicle.D.val(next),
        val + 1)
...
cronicle.minute = cronicle.m = {
   ...
   */
  val: function (d) {
    return d.m || (d.m = cronicle.date.getMin.call(d))
  },
   ...
   */
  start: function (d) {
    return d.mStart || (d.mStart = cronicle.date.next(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d), cronicle.h.val(d), cronicle.m.val(d)))
  },
   ...
   */
    ...
    return d.mEnd || (d.mEnd = cronicle.date.prev(
      cronicle.Y.val(d), cronicle.M.val(d), cronicle.D.val(d), cronicle.h.val(d), cronicle.m.val(d)))
  },
   ...
   */
  next: function (d, val) {
    var m = cronicle.m.val(d)

    var s = cronicle.s.val(d)

    var inc = val > 59 ? 60 - m : (val <= m ? (60 - m) + val : val - m)

    var next = new Date(d.getTime() + (inc * cronicle.MIN) - (s * cronicle.SEC))

    // correct for passing over a daylight savings boundry
    if (!cronicle.date.isUTC && next.getTime() <= d.getTime()) {
      next = new Date(d.getTime() + ((inc + 120) * cronicle.MIN) - (s * cronicle.SEC))
    }
  ...
  prev: function (d, val) {
    ...
    return cronicle.date.prev(
      ...
      cronicle.M.val(d),
      cronicle.D.val(d),
      cronicle.h.val(d) + (val >= cronicle.m.val(d) ? -1 : 0),
      val)
  }
...
cronicle.month = cronicle.M = {
   ...
   */
  val: function (d) {
    return d.M || (d.M = cronicle.date.getMonth.call(d) + 1)
  },
   ...
   */
  start: function (d) {
    return d.MStart || (d.MStart = cronicle.date.next(cronicle.Y.val(d), cronicle.M.val(d)))
  },
   ...
   */
    ...
    return d.MEnd || (d.MEnd = cronicle.date.prev(cronicle.Y.val(d), cronicle.M.val(d)))
  },
  ...
  next: function (d, val) {
    ...
    return cronicle.date.next(
      cronicle.Y.val(d) + (val > cronicle.M.val(d) ? 0 : 1),
      val)
  },
  ...
  prev: function (d, val) {
    ...
    return cronicle.date.prev(
      cronicle.Y.val(d) - (val >= cronicle.M.val(d) ? 1 : 0),
      val)
  }
...
cronicle.second = cronicle.s = {
   ...
   */
  val: function (d) {
    return d.s || (d.s = cronicle.date.getSec.call(d))
  },
  ...
  next: function (d, val) {
    ...
    var next = new Date(d.getTime() + (inc * cronicle.SEC))

    // correct for passing over a daylight savings boundry
    if (!cronicle.date.isUTC && next.getTime() <= d.getTime()) {
      next = new Date(d.getTime() + ((inc + 7200) * cronicle.SEC))
    }
  ...
  prev: function (d, val, cache) {
    ...
    return cronicle.date.prev(
      ...
      cronicle.D.val(d),
      cronicle.h.val(d),
      cronicle.m.val(d) + (val >= cronicle.s.val(d) ? -1 : 0),
      val)
  }
...
cronicle.time = cronicle.t = {
  ...
  val: function (d) {
    return d.t || (d.t =
      (cronicle.h.val(d) * 3600) + (cronicle.m.val(d) * 60) + (cronicle.s.val(d)))
  },
  ...
  next: function (d, val) {
    ...
    var next = cronicle.date.next(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      cronicle.D.val(d) + (val <= cronicle.t.val(d) ? 1 : 0),
      0,
      0,
      val)
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.weekOfMonth = cronicle.wm = {
  ...
  val: function (d) {
    return d.wm || (d.wm =
      (cronicle.D.val(d) +
        (cronicle.dw.val(cronicle.M.start(d)) - 1) + (7 - cronicle.dw.val(d))) / 7)
  },
   ...
   */
  isValid: function (d, val) {
    return cronicle.wm.val(d) === (val || cronicle.wm.extent(d)[1])
  },
  ...
  extent: function (d) {
    return d.wmExtent || (d.wmExtent = [1,
      (cronicle.D.extent(d)[1] + (cronicle.dw.val(cronicle.M.start(d)) - 1) +
        (7 - cronicle.dw.val(cronicle.M.end(d)))) / 7])
  },
   ...
   */
  start: function (d) {
    return d.wmStart || (d.wmStart = cronicle.date.next(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      Math.max(cronicle.D.val(d) - cronicle.dw.val(d) + 1, 1)))
  },
   ...
   */
    ...
    return d.wmEnd || (d.wmEnd = cronicle.date.prev(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      Math.min(cronicle.D.val(d) + (7 - cronicle.dw.val(d)), cronicle.D.extent(d)[1])))
  },
   ...
   */
  next: function (d, val) {
    val = val > cronicle.wm.extent(d)[1] ? 1 : val

    var month = cronicle.date.nextRollover(d, val, cronicle.wm, cronicle.M)

    var wmMax = cronicle.wm.extent(month)[1]
    ...
    return cronicle.date.next(
      cronicle.Y.val(month),
      cronicle.M.val(month),
      Math.max(1, (val - 1) * 7 - (cronicle.dw.val(month) - 2)))
  },
   ...
   */
  prev: function (d, val) {
    var month = cronicle.date.prevRollover(d, val, cronicle.wm, cronicle.M)
    ...
    return cronicle.wm.end(cronicle.date.next(
      cronicle.Y.val(month),
      cronicle.M.val(month),
      Math.max(1, (val - 1) * 7 - (cronicle.dw.val(month) - 2))))
  }
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.weekOfYear = cronicle.wy = {
  ...
  val: function (d) {
    ...
    var wThur = cronicle.dw.next(cronicle.wy.start(d), 5)

    var YThur = cronicle.dw.next(cronicle.Y.prev(wThur, cronicle.Y.val(wThur) - 1), 5)

    // caculate the difference between the two dates in weeks
    return (d.wy = 1 + Math.ceil((wThur.getTime() - YThur.getTime()) / cronicle.WEEK))
  },
   ...
   */
  isValid: function (d, val) {
    return cronicle.wy.val(d) === (val || cronicle.wy.extent(d)[1])
  },
  ...
  extent: function (d) {
    ...
    var year = cronicle.dw.next(cronicle.wy.start(d), 5)

    var dwFirst = cronicle.dw.val(cronicle.Y.start(year))

    var dwLast = cronicle.dw.val(cronicle.Y.end(year))
   ...
   */
  start: function (d) {
    return d.wyStart || (d.wyStart = cronicle.date.next(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      // jump to the Monday of the current week
      cronicle.D.val(d) - (cronicle.dw.val(d) > 1 ? cronicle.dw.val(d) - 2 : 6)
    ))
  },
   ...
   */
    ...
    return d.wyEnd || (d.wyEnd = cronicle.date.prev(
      cronicle.Y.val(d),
      cronicle.M.val(d),
      // jump to the Saturday of the current week
      cronicle.D.val(d) + (cronicle.dw.val(d) > 1 ? 8 - cronicle.dw.val(d) : 0)
    ))
  },
   ...
   */
  next: function (d, val) {
    val = val > cronicle.wy.extent(d)[1] ? 1 : val

    var wyThur = cronicle.dw.next(cronicle.wy.start(d), 5)

    var year = cronicle.date.nextRollover(wyThur, val, cronicle.wy, cronicle.Y)

    // handle case where 1st of year is last week of previous month
    if (cronicle.wy.val(year) !== 1) {
      year = cronicle.dw.next(year, 2)
    }

    var wyMax = cronicle.wy.extent(year)[1]

    var wyStart = cronicle.wy.start(year)

    val = val > wyMax ? 1 : val || wyMax

    return cronicle.date.next(
      cronicle.Y.val(wyStart),
      cronicle.M.val(wyStart),
      cronicle.D.val(wyStart) + 7 * (val - 1)
    )
  },
  ...
  prev: function (d, val) {
    ...
    var year = cronicle.date.prevRollover(wyThur, val, cronicle.wy, cronicle.Y)
    ...
    var wyEnd = cronicle.wy.end(year)

    val = val > wyMax ? wyMax : val || wyMax

    return cronicle.wy.end(cronicle.date.next(
      cronicle.Y.val(wyEnd),
      cronicle.M.val(wyEnd),
      cronicle.D.val(wyEnd) + 7 * (val - 1)
    ))
  }
}
/**
...
cronicle.year = cronicle.Y = {
   ...
   */
  val: function (d) {
    return d.Y || (d.Y = cronicle.date.getYear.call(d))
  },
   ...
   */
  isValid: function (d, val) {
    return cronicle.Y.val(d) === val
  },

  /**
   * The minimum and maximum valid values for the year constraint.
   * If max is past 2099, cronicle.D.extent must be fixed to calculate leap years
   * correctly.
   */
  extent: function () {
    return [1970, 2099]
   ...
   */
  start: function (d) {
    return d.YStart || (d.YStart = cronicle.date.next(cronicle.Y.val(d)))
  },
   ...
   */
    ...
    return d.YEnd || (d.YEnd = cronicle.date.prev(cronicle.Y.val(d)))
  },
   ...
   */
  next: function (d, val) {
    return val > cronicle.Y.val(d) && val <= cronicle.Y.extent()[1]
      ? cronicle.date.next(val) : cronicle.NEVER
  },
   ...
   */
  prev: function (d, val) {
    return val < cronicle.Y.val(d) && val >= cronicle.Y.extent()[0]
      ? cronicle.date.prev(val) : cronicle.NEVER
  }
...
cronicle.fullDate = cronicle.fd = {
   ...
   */
  next: function (d, val) {
    return cronicle.fd.val(d) < val ? new Date(val) : cronicle.NEVER
  },
 ...
 */
cronicle.modifier.after = cronicle.modifier.a = function (constraint, values) {
  var value = values[0]
 ...
 */
cronicle.modifier.before = cronicle.modifier.b = function (constraint, values) {
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.compile = function (schedDef) {
  var constraints = []
  ...
  for (var key in schedDef) {
    ...
    var constraint = mod ? cronicle.modifier[mod](cronicle[name], vals) : cronicle[name]
  ...
  return {
    ...
    start: function (dir, startDate) {
      ...
      var nextVal = cronicle.array[dir]
     ...
     */
      ...
      var nextVal = cronicle.array[dir + 'Invalid']
    ...
    tick: function (dir, date) {
      return new Date(dir === 'next'
        ? tickConstraint.end(date).getTime() + cronicle.SEC
        : tickConstraint.start(date).getTime() - cronicle.SEC)
    },
 ...
 *     http://github.com/bunkat/cronicle
 */
cronicle.schedule = function (sched) {
  ...
  for (var i = 0; i < schedulesLen; i++) {
    schedules.push(cronicle.compile(sched.schedules[i]))
  }

  for (var j = 0; j < exceptionsLen; j++) {
    exceptions.push(cronicle.compile(sched.exceptions[j]))
  }
  ...
  function getInstances (dir, count, startDate, endDate, isRange) {
    ...
    while (maxAttempts-- && loopCount && (next = findNext(schedStarts, compare))) {
      ...
      if (isRange) {
        ...
        var r = isForward
          ...
          : [
            end ? (new Date(endDate ? Math.max(endDate, end.getTime() + cronicle.SEC) : end.getTime() + cronicle.SEC)) : undefined,
            new Date(Math.min(startDate, next.getTime() + cronicle.SEC))
          ]
    ...
    return results.length === 0 ? cronicle.NEVER : count === 1 ? results[0] : results
  }
  ...
  function getAllInstances (startDate, endDate) {
    ...
    return results.length === 0 ? cronicle.NEVER : results
  }
  ...
  return {
     ...
     */
    isValid: function (d) {
      return getInstances('next', 1, d, d) !== cronicle.NEVER
    },
 ...
 * (c) 2013 Bill, BunKat LLC.
 *
 * Works similar to setTimeout() but allows you to specify a cronicle schedule
...
cronicle.setTimeout = function (fn, sched) {
  var s = cronicle.schedule(sched); var t
  if (fn) {
    scheduleTimeout()
  }
 ...
 * (c) 2013 Bill, BunKat LLC.
 *
 * Works similar to setInterval() but allows you to specify a cronicle schedule
 * instead of milliseconds.
 *
 * cronicle is freely distributable under the MIT license.
 * For all details and documentation:
...
cronicle.setInterval = function (fn, sched) {
  if (!fn) {
    return
  }

  var t = cronicle.setTimeout(scheduleTimeout, sched)
  ...
  function scheduleTimeout () {
    if (!done) {
      fn()
      t = cronicle.setTimeout(scheduleTimeout, sched)
    }
  }
 ...
 * (c) 2013 Bill, BunKat LLC.
 *
 * Configures helper functions to switch between using local time and UTC. cronicle
...
cronicle.date.timezone = function (useLocalTime) {
  // configure the date builder used to create new dates in the right timezone
  cronicle.date.build = useLocalTime
  ...
  cronicle.date.getYear = d[get + 'FullYear']
  cronicle.date.getMonth = d[get + 'Month']
  cronicle.date.getDate = d[get + 'Date']
  cronicle.date.getDay = d[get + 'Day']
  cronicle.date.getHour = d[get + 'Hours']
  cronicle.date.getMin = d[get + 'Minutes']
  cronicle.date.getSec = d[get + 'Seconds']

  // set the isUTC flag
  cronicle.date.isUTC = !useLocalTime
}

// friendly names for available timezones
cronicle.date.UTC = function () {
  cronicle.date.timezone(false)
}
cronicle.date.localTime = function () {
  cronicle.date.timezone(true)
}
...
cronicle.SEC = 1000
cronicle.MIN = cronicle.SEC * 60
cronicle.HOUR = cronicle.MIN * 60
cronicle.DAY = cronicle.HOUR * 24
cronicle.WEEK = cronicle.DAY * 7

// Array of days in each month, must be corrected for leap years
cronicle.DAYS_IN_MONTH = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
 ...
 */
cronicle.date.next = function (Y, M, D, h, m, s) {
  return cronicle.date.build(
    Y,
    M !== undefined ? M - 1 : 0,
    D !== undefined ? D : 1,
    h || 0,
...
cronicle.date.nextRollover = function (d, val, constraint, period) {
  var cur = constraint.val(d)

  var max = constraint.extent(d)[1]

  return (((val || max) <= cur) || val > max)
    ? new Date(period.end(d).getTime() + cronicle.SEC)
    : period.start(d)
}
/**
 * Prev
 ...
 */
cronicle.date.prev = function (Y, M, D, h, m, s) {
  var len = arguments.length
  M = len < 2 ? 11 : M - 1
  D = len < 3 ? cronicle.D.extent(cronicle.date.next(Y, M + 1))[1] : D
  h = len < 4 ? 23 : h
  m = len < 5 ? 59 : m
  s = len < 6 ? 59 : s

  return cronicle.date.build(Y, M, D, h, m, s)
}
/**
 * Prev Rollover
 * (c) 2013 Bill, BunKat LLC.
...
cronicle.date.prevRollover = function (d, val, constraint, period) {
  var cur = constraint.val(d)
